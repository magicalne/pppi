...
// as the session does). The omni conversation itself stays a separate `pi
// --mode rpc` child (the launcher form; `/omni here` in a later phase drives
// the host session directly). Voice runs in a spawned audio-service child so
// native STT/silero/kokoro never load into pi's process.
//
...
export async function startOmniHost(notify: (message: string, level?: "info" | "error") => void): Promise<void> {
	if (running) {
	...
	const lines = [
		`pppi gateway is up (while this session lives) — omni session: ${omniSession}`,
		pair ? `\n${pairUrl(pair)}?pair=${pair.token}` : "",
		`\nvoice: ${audio ? "audio service child" : "unavailable (bun not found)"}`,
		"stop: this session ending closes the gateway; the standalone cli stays available.",
	];

...
│  terminal pi ──┐                                              │
│                │   ┌─────────────┐    RPC JSONL (stdio)       │
│  web ──WS──►   ├───│  pppi server │◄──► omni session         │
...
1. **The omni agent is a pi session, not a service.** It gets the omni
   ...
   (`pi --mode rpc --session-id pppi-omni`), an interactive instance is just
...
3. **Voice is transcribed on the Mac.** Clients upload 16 kHz mono PCM16
   ...
   (resolved: `$PPPI_STT_MODEL` → pi-transcribe's config → HF cache).
...
- Gateway restarts → `--session-id pppi-omni` resumes the same session file.
